...
func Timeout(timeout time.Duration, errTimeout *errors.AffordAbodeError) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		case <-panicChan:
			...
			affordAbodeErr := errors.NewInternalServerError("did not recover from panic in side timeout")
			tw.ResponseWriter.WriteHeader(affordAbodeErr.Status)
			eResp, _ := json.Marshal(affordAbodeErr)
